...
import com.inventory.form.input.FormInputSatuan;
...
import com.inventory.tablemodel.TabModSatuan;
...
public class FormSatuan extends Form{
    private static final long serialVersionUID = 1L;


    private final SatuanController controller = new SatuanController();
    private final TabModSatuan tblModel = new TabModSatuan();
    ...
    private JPanel setInfo() {
        ...
        imageLogo.setIcon(new FlatSVGIcon(AppResources.ICON_BASE + "satuan.svg", 0.4f));
        
        JLabel lbTItle = new JLabel("Data Satuan");
        lbTItle.putClientProperty(FlatClientProperties.STYLE, "font:bold 17");
        
        JLabel lbBreadcrumb = new JLabel("MASTER > Satuan");
    ...
    private void insertData() {
        ...
        ModalDialog.showModal(this, 
                new SimpleModalBorder(new FormInputSatuan(null, this),"Tambah Satuan"), option,"form input");
    }

    private void updateData() {
        int row = tblData.getSelectedRow();
        if(row != -1){
            Satuan model = tblModel.getData(row);
            Option option = ModalDialog.createOption();
            option.setBackground(Color.BLACK);
            ModalDialog.showModal(this, 
                    new SimpleModalBorder(new FormInputSatuan(model, this),"Perbarui Satuan"), option,"form update");
        }else{
                    ...
                    new MessageModal(
                            ...
                            (control, action) -> {
                                if(action == SimpleModalBorder.YES_OPTION){
                                    model.setDeleteBy(loggedInUser.getUserID());
            ...
            ModalDialog.showModal(this, new MessageModal(MessageModal.Type.INFO, message, title, ModalBorder.YES_NO_OPTION, 
                (control, action) -> {
                    if (action == ModalBorder.YES_OPTION) {
                        boolean success = controller.restoreData(model.getSatuanID(), model.getSatuanName());
    ...
    private void exportToExcel() {
        List<Satuan> data = controller.getData(false, Integer.MAX_VALUE, 1);
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            Sheet sheet = workbook.createSheet("Satuan");

            // Header
            Row header = sheet.createRow(0);
            header.createCell(0).setCellValue("No");
            header.createCell(1).setCellValue("Nama Satuan");
            header.createCell(2).setCellValue("Nama Keterangan");
            ...
            for (Satuan s : data) {
                ...
                row.createCell(1).setCellValue(s.getSatuanName());
                row.createCell(2).setCellValue(s.getKeterangan());
            }
