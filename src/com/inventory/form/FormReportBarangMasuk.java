...
import com.inventory.tablemodel.TabModReportBarangMasuk;
...
public class FormReportBarangMasuk extends Form{
    private static final long serialVersionUID = 1L;


    private final ReportController controller = new ReportController();
    private final TabModReportBarangMasuk tblModel = new TabModReportBarangMasuk();
    ...
    private DatePicker pickerBarangMasukDate;
    private JFormattedTextField txtBarangMasukDate;
    
    private Date startDate;
    private Date endDate;
    
    public FormReportBarangMasuk() {
    ...
    private JPanel setInfo() {
        ...
        JLabel lbTitle = new JLabel("Data Laporan Barang Masuk");
        lbTitle.putClientProperty(FlatClientProperties.STYLE, "font:bold 17");
        
        JLabel lbBreadcrumb = new JLabel("LAPORAN > Laporan Barang Masuk");
    ...
    private JPanel setButton() {
        ...
        pickerBarangMasukDate = new DatePicker();
        pickerBarangMasukDate.setDateSelectionMode(DatePicker.DateSelectionMode.BETWEEN_DATE_SELECTED);
        pickerBarangMasukDate.setUsePanelOption(true);
        pickerBarangMasukDate.setCloseAfterSelected(true);
        txtBarangMasukDate = new JFormattedTextField();
        txtBarangMasukDate.setHorizontalAlignment(JFormattedTextField.CENTER);
        pickerBarangMasukDate.setEditor(txtBarangMasukDate);
        ...
        btnPreview.addActionListener((e) -> {
            controller.exportBarangMasuk(startDate,endDate,ExportType.PREVIEW);
        });
        ...
        btnPDF.addActionListener((e) -> {
            controller.exportBarangMasuk(startDate,endDate,ExportType.PDF);
        });
        ...
        btnExcel.addActionListener((e) -> {
            controller.exportBarangMasuk(startDate,endDate,ExportType.EXCEL);
        });
        ...
        panel.add(txtBarangMasukDate,"hmin 30, wmin 200");
    ...
    private boolean hasValidDateSelection() {
        LocalDate[] dates = pickerBarangMasukDate.getSelectedDateRange();
    ...
    private void updateTable(boolean isSearch) {
        ...
        int totalItems = controller.getTotalItemsBarangMasuk(startDate, endDate, keyword);
        pagination.setTotalItems(totalItems);

        List<BarangMasukDetail> list = controller.getDataBarangMasuk(startDate, endDate, keyword, pagination.getItemsPerPage(), pagination.getCurrentPage());
    ...
    private void resetDate(){
        disableComponent();
        pickerBarangMasukDate.clearSelectedDate();
        txtBarangMasukDate.setValue(null);
        tblModel.clear();
        pagination.firstPage();
    }
