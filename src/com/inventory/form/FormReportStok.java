...
import com.inventory.tablemodel.TabModReportStok;
...
public class FormReportStok extends Form{
    private static final long serialVersionUID = 1L;


    private final ReportController controller = new ReportController();
    private final TabModReportStok tblModel = new TabModReportStok();
    ...
    private JPanel setInfo() {
        ...
        imageLogo.setIcon(new FlatSVGIcon(AppResources.ICON_BASE + "stok.svg", 0.4f));
        
        JLabel lbTItle = new JLabel("Data Laporan Stok Barang");
        lbTItle.putClientProperty(FlatClientProperties.STYLE, "font:bold 17");
        
        JLabel lbBreadcrumb = new JLabel("LAPORAN > Laporan Stok");
    ...
    private JPanel setButton() {
        ...
        btnPreview.addActionListener((e) -> {
            controller.exportStok(ExportType.PREVIEW);
        });
        ...
        btnPDF.addActionListener((e) -> {
            controller.exportStok(ExportType.PDF);
        });
        ...
        btnExcel.addActionListener((e) -> {
            controller.exportStok(ExportType.EXCEL);
        });
    ...
    private void updateTable(boolean isSearch) {
        ...
        int totalItems = controller.getTotalItemsStok(keyword);
        pagination.setTotalItems(totalItems);

        List<BarangMasukDetail> list;
        if (isSearch) {
            list = controller.getDataStok(keyword, pagination.getItemsPerPage(), pagination.getCurrentPage());
        } else {
            list = controller.getDataStok(null,pagination.getItemsPerPage(), pagination.getCurrentPage());
        }
