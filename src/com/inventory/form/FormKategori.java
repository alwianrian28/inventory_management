...
import com.inventory.form.input.FormInputKategori;
...
import com.inventory.tablemodel.TabModKategori;
...
public class FormKategori extends Form{
    ...
    private final TabModKategori tblModel = new TabModKategori();
    ...
    private JPanel setInfo() {
        ...
        imageLogo.setIcon(new FlatSVGIcon(AppResources.ICON_BASE + "kategori.svg", 0.4f));
        
        JLabel lbTItle = new JLabel("Data Kategori");
        lbTItle.putClientProperty(FlatClientProperties.STYLE, "font:bold 17");
        
        JLabel lbBreadcrumb = new JLabel("MASTER > Kategori");
    ...
    private void insertData() {
        ...
        ModalDialog.showModal(this, 
                new SimpleModalBorder(new FormInputKategori(null, this),"Tambah Kategori"), option,"form input");
    }

    private void updateData() {
        int row = tblData.getSelectedRow();
        if(row != -1){
            Kategori model = tblModel.getData(row);
            Option option = ModalDialog.createOption();
            option.setBackground(Color.BLACK);
            ModalDialog.showModal(this, 
                    new SimpleModalBorder(new FormInputKategori(model, this),"Perbarui Kategori"), option,"form update");
        }else{
            ...
            ModalDialog.showModal(this, new MessageModal(MessageModal.Type.INFO, message, title, ModalBorder.YES_NO_OPTION, 
                (control, action) -> {
                    if (action == ModalBorder.YES_OPTION) {
                        boolean success = controller.restoreData(model.getKategoriID(), model.getKategoriName());
    ...
    private void exportToExcel() {
        List<Kategori> data = controller.getData(false, Integer.MAX_VALUE, 1);
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            Sheet sheet = workbook.createSheet("Kategori");

            // Header
            Row header = sheet.createRow(0);
            header.createCell(0).setCellValue("No");
            header.createCell(1).setCellValue("Nama Kategori");
            ...
            for (Kategori k : data) {
                ...
                row.createCell(1).setCellValue(k.getKategoriName());
