...
import com.inventory.form.input.FormInputBarang;
...
import com.inventory.util.ImageRenderer;
...
public class FormBarang extends Form{
    ...
    private JPanel setInfo() {
        ...
        imageLogo.setIcon(new FlatSVGIcon(AppResources.ICON_BASE + "barang.svg", 0.4f));
        
        JLabel lbTItle = new JLabel("Data Barang");
        lbTItle.putClientProperty(FlatClientProperties.STYLE, "font:bold 17");
        
        JLabel lbBreadcrumb = new JLabel("MASTER > Barang");
    ...
    private void setTableProperties() {
        tblData.getColumnModel().getColumn(11).setCellRenderer(new ImageRenderer());

        TabelUtils.setColumnWidths(tblData, new int[]{0}, new int[]{50});
        TabelUtils.setHeaderAlignment(tblData, new int[]{0,11}, new int[]{JLabel.CENTER,JLabel.CENTER}, JLabel.LEFT);
    ...
    private void insertData() {
        ...
        ModalDialog.showModal(this, 
                new SimpleModalBorder(new FormInputBarang(null, this),"Tambah Barang"), option,"form input");
    }
    ...
    private void updateData() {
        ...
        if(row != -1){
            ...
            option.setBackground(Color.BLACK);
            ModalDialog.showModal(this, 
                    new SimpleModalBorder(new FormInputBarang(model, this),"Perbarui Barang"), option,"form update");
        }else{
            ...
            ModalDialog.showModal(this, new MessageModal(MessageModal.Type.INFO, message, title, ModalBorder.YES_NO_OPTION, 
                (control, action) -> {
                    if (action == ModalBorder.YES_OPTION) {
                        boolean success = controller.restoreData(model.getBarangID(), model.getBarangCode(), model.getBarcode());
    ...
    private void exportToExcel() {
        List<Barang> data = controller.getData(false, Integer.MAX_VALUE, 1);
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            header.createCell(1).setCellValue("Kode Barang");
            ...
            header.createCell(5).setCellValue("Kategori");
            header.createCell(6).setCellValue("Satuan");
            header.createCell(7).setCellValue("Harga Beli");
            header.createCell(8).setCellValue("Harga Jual");
            header.createCell(9).setCellValue("Stok");
            header.createCell(10).setCellValue("Stok Minimum");
            ...
            for (Barang b : data) {
                ...
                row.createCell(1).setCellValue(b.getBarangCode());
                row.createCell(2).setCellValue(b.getBarangName());
                row.createCell(3).setCellValue(b.getBarcode());
                row.createCell(4).setCellValue(b.getMerk().getMerkName());
                row.createCell(5).setCellValue(b.getKategori().getKategoriName());
                row.createCell(6).setCellValue(b.getSatuan().getSatuanName());
                row.createCell(7).setCellValue(b.getHargaJual());
                row.createCell(8).setCellValue(b.getStokMinimum());
                row.createCell(9).setCellValue(b.getSupplier().getSupplierName());
            }
