...
import com.inventory.form.input.FormInputGudang;
...
import com.inventory.tablemodel.TabModGudang;
...
public class FormGudang extends Form{
    private static final long serialVersionUID = 1L;


    private final GudangController controller = new GudangController();
    private final TabModGudang tblModel = new TabModGudang();
    ...
    private JPanel setInfo() {
        ...
        imageLogo.setIcon(new FlatSVGIcon(AppResources.ICON_BASE + "gudang.svg", 0.4f));
        
        JLabel lbTItle = new JLabel("Data Gudang");
        lbTItle.putClientProperty(FlatClientProperties.STYLE, "font:bold 17");
        
        JLabel lbBreadcrumb = new JLabel("MASTER > Gudang");
    ...
    private void insertData() {
        ...
        ModalDialog.showModal(this, 
                new SimpleModalBorder(new FormInputGudang(null, this),"Tambah Gudang"), option,"form input");
    }

    private void updateData() {
        int row = tblData.getSelectedRow();
        if(row != -1){
            Gudang model = tblModel.getData(row);
            Option option = ModalDialog.createOption();
            option.setBackground(Color.BLACK);
            ModalDialog.showModal(this, 
                    new SimpleModalBorder(new FormInputGudang(model, this),"Perbarui Gudang"), option,"form update");
        }else{
            ...
            ModalDialog.showModal(this, new MessageModal(MessageModal.Type.INFO, message, title, ModalBorder.YES_NO_OPTION, 
                (control, action) -> {
                    if (action == ModalBorder.YES_OPTION) {
                        boolean success = controller.restoreData(model.getGudangID(), model.getGudangCode(), model.getGudangName());
    ...
    private void exportToExcel() {
        List<Gudang> data = controller.getData(false, Integer.MAX_VALUE, 1);
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            Sheet sheet = workbook.createSheet("Gudang");

            // Header
            Row header = sheet.createRow(0);
            header.createCell(0).setCellValue("No");
            header.createCell(1).setCellValue("Kode Gudang");
            header.createCell(2).setCellValue("Nama Gudang");
            header.createCell(3).setCellValue("Alamat");
            header.createCell(4).setCellValue("Keterangan");
            ...
            for (Gudang g : data) {
                ...
                row.createCell(1).setCellValue(g.getGudangCode());
                row.createCell(2).setCellValue(g.getGudangName());
                row.createCell(3).setCellValue(g.getAlamat());
                row.createCell(4).setCellValue(g.getKeterangan());
            }
