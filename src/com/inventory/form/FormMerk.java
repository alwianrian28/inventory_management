...
import com.inventory.form.input.FormInputMerk;
...
import com.inventory.tablemodel.TabModMerk;
...
public class FormMerk extends Form{
    private static final long serialVersionUID = 1L;


    private final MerkController controller = new MerkController();
    private final TabModMerk tblModel = new TabModMerk();
    ...
    private JPanel setInfo() {
        ...
        imageLogo.setIcon(new FlatSVGIcon(AppResources.ICON_BASE + "merk.svg", 0.4f));
        
        JLabel lbTItle = new JLabel("Data Merk");
        lbTItle.putClientProperty(FlatClientProperties.STYLE, "font:bold 17");
        
        JLabel lbBreadcrumb = new JLabel("MASTER > Merk");
    ...
    private void insertData() {
        ...
        ModalDialog.showModal(this, 
                new SimpleModalBorder(new FormInputMerk(null, this),"Tambah Merk"), option,"form input");
    }

    private void updateData() {
        int row = tblData.getSelectedRow();
        if(row != -1){
            Merk model = tblModel.getData(row);
            Option option = ModalDialog.createOption();
            option.setBackground(Color.BLACK);
            ModalDialog.showModal(this, 
                    new SimpleModalBorder(new FormInputMerk(model, this),"Perbarui Merk"), option,"form update");
        }else{
            ...
            ModalDialog.showModal(this, new MessageModal(MessageModal.Type.INFO, message, title, ModalBorder.YES_NO_OPTION, 
                (control, action) -> {
                    if (action == ModalBorder.YES_OPTION) {
                        boolean success = controller.restoreData(model.getMerkID(), model.getMerkName());
    ...
    private void exportToExcel() {
        List<Merk> data = controller.getData(false, Integer.MAX_VALUE, 1);
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            Sheet sheet = workbook.createSheet("Merk");

            // Header
            Row header = sheet.createRow(0);
            header.createCell(0).setCellValue("No");
            header.createCell(1).setCellValue("Nama Merk");
            ...
            for (Merk m : data) {
                ...
                row.createCell(1).setCellValue(m.getMerkName());
                row.createCell(2).setCellValue(m.getKeterangan());
            }
