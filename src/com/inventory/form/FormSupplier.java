...
import com.inventory.form.input.FormInputSupplier;
...
import com.inventory.tablemodel.TabModSupplier;
...
public class FormSupplier extends Form{
    ...
    private final TabModSupplier tblModel = new TabModSupplier();
    ...
    private JPanel setInfo() {
        ...
        imageLogo.setIcon(new FlatSVGIcon(AppResources.ICON_BASE + "supplier.svg", 0.4f));
        
        JLabel lbTItle = new JLabel("Data Supplier");
        lbTItle.putClientProperty(FlatClientProperties.STYLE, "font:bold 17");
        
        JLabel lbBreadcrumb = new JLabel("MASTER > Supplier");
    ...
    private void insertData() {
        ...
        ModalDialog.showModal(this, 
                new SimpleModalBorder(new FormInputSupplier(null, this),"Tambah Supplier"), option,"form input");
    }

    private void updateData() {
        int row = tblData.getSelectedRow();
        if(row != -1){
            Supplier model = tblModel.getData(row);
            Option option = ModalDialog.createOption();
            option.setBackground(Color.BLACK);
            ModalDialog.showModal(this, 
                    new SimpleModalBorder(new FormInputSupplier(model, this),"Perbarui Supplier"), option,"form update");
        }else{
            ...
            ModalDialog.showModal(this, new MessageModal(MessageModal.Type.INFO, message, title, ModalBorder.YES_NO_OPTION, 
                (control, action) -> {
                    if (action == ModalBorder.YES_OPTION) {
                        boolean success = controller.restoreData(model.getSupplierID(), model.getSupplierName());
    ...
    private void exportToExcel() {
        List<Supplier> data = controller.getData(false, Integer.MAX_VALUE, 1);
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            header.createCell(1).setCellValue("Nama Supplier");
            ...
            for (Supplier s : data) {
                ...
                row.createCell(2).setCellValue(s.getTelepon());
                row.createCell(3).setCellValue(s.getEmail());
                row.createCell(4).setCellValue(s.getAlamat());
            }
